...
def parse_config():
    ...
    parser.add_argument("--config", default="./configs/DQnet/DQnet.py", type=str)
    ...
    for curr_epoch in range(start_epoch, cfg.train.num_epochs):
        ...
        for batch_idx, batch in enumerate(tr_loader):
            ...
            for scale in multiscales:
                if scale != 1:
                    data["image1.0"] = F.interpolate(batch_data["image1.0"], scale_factor=scale, mode='bicubic', align_corners=False)
                    data["mask"] = F.interpolate(batch_data["mask"], scale_factor=scale, mode='bicubic', align_corners=False)
                else:
                    data = batch_data
...
def main():
    ...
    setup_for_distributed(cfg.local_rank==0)
