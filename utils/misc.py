...
# def to_device_parallel(data, device):
#     """
#     :param data:
#     :param device:
#     :return:
...
#     if isinstance(data, (tuple, list)):
#         ctor = tuple if isinstance(data, tuple) else list
#         return ctor(to_device_parallel(d, device) for d in data)
#     elif isinstance(data, dict):
#         return {name: to_device_parallel(item, device) for name, item in data.items()}
#     elif isinstance(data, torch.Tensor):
#         return data.cuda()
#     else:
#         raise TypeError(f"Unsupported type {type(data)}. Only support Tensor or tuple/list/dict containing Tensors.")
